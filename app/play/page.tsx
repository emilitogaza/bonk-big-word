...
function PlayGame() {
  ...
  // Listen for manual query parameters and update localStorage for adjusting the team manually
  useEffect(() => {
    const teamParam = searchParams.get("team") as Team | null;
    if (teamParam && (teamParam === "orange" || teamParam === "purple")) {
      localStorage.setItem("currentTeam", teamParam);
      setCurrentTeam(teamParam);
      // Then we clean the url from the manual user entered query parameter
